...
int main() {
  ...
  h.onMessage([&lane, &initial_vel, &map_waypoints_x, &map_waypoints_y,
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          if (front_car) { // There is car ahead of ego vehicle
            if (!leftlane_car &&
                lane > 0) { // left lane present and no car in left lane
              lane--;       // Change lane
            } else if (!rightlane_car &&
                       lane !=
                           2) { // right lane present and no car in right lane
              lane++;           // Change lane
            } else {
              initial_vel -= max_acc;
            }
          ...
          // We will use car's current position as reference point or previous
          // path's end point as reference point
          double ref_x = car_x;
          double ref_y = car_y;
          double ref_yaw = deg_rad(car_yaw);

          // If prev path has few elements, use car's current position as
          // reference point
          if (prev_size < 2) {
          ...
          vector<double> next_wp0 =
              Cartesian(car_s + 30, 2 + 4 * lane, map_waypoints_s,
                        map_waypoints_x, map_waypoints_y);
          vector<double> next_wp1 =
              Cartesian(car_s + 60, 2 + 4 * lane, map_waypoints_s,
                        map_waypoints_x, map_waypoints_y);
          vector<double> next_wp2 =
              Cartesian(car_s + 90, 2 + 4 * lane, map_waypoints_s,
                        map_waypoints_x, map_waypoints_y);
